...
export async function renderDashboardView(productId) {
  ...
  // Create gradient from product colors
  const colors = product.colors || ['#1a202c', '#2d3748', '#4a5568'];
  const gradientStyle = `background: linear-gradient(135deg, ${colors[0]} 0%, ${colors[1]} 50%, ${colors[2]} 100%);`;
  ...
  spaRoot.innerHTML = `
    <div class="min-h-screen" style="${gradientStyle}">
      <!-- Header con botón de regreso -->
      <div class="relative z-10 p-6">
        <button id="back-to-products" class="inline-flex items-center px-4 py-2 bg-white/10 backdrop-blur-sm text-white rounded-lg hover:bg-white/20 transition-all duration-300 shadow-lg">
        ...
        <div class="max-w-4xl mx-auto">
          ...
          <div class="grid md:grid-cols-3 gap-6 mb-12">
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center">
              ...
              <div class="text-white/80">Progreso Completado</div>
            </div>

            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center">
              <div class="text-3xl font-bold text-white mb-2">0</div>
              <div class="text-white/80">Preguntas Respondidas</div>
            </div>

            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center">
              <div class="text-3xl font-bold text-white mb-2">--</div>
              <div class="text-white/80">Mejor Puntuación</div>
            </div>
          </div>

          <!-- Main Action Card -->
          <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-8 text-center mb-8">
            ...
            <button id="start-training" class="bg-white text-gray-900 px-8 py-4 rounded-xl font-bold text-lg hover:bg-gray-100 transition-all duration-300 shadow-lg transform hover:scale-105">
          ...
          <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-8 mb-8">
            ...
            <div class="grid md:grid-cols-2 gap-6">
              ${product.detailedFeatures?.slice(0, 6).map(feature => {
                ...
                `;
              }).join('') || '<p class="text-white/80 text-center">Características en desarrollo...</p>'}
            </div>
          </div>
          ...
          <div class="grid md:grid-cols-2 gap-6">
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center">
              <h4 class="text-lg font-semibold text-white mb-4">Recursos Adicionales</h4>
              <button class="text-white/80 hover:text-white transition-colors">
                ...
                <span class="block text-sm">Documentación</span>
              </button>
            </div>

            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center">
              <h4 class="text-lg font-semibold text-white mb-4">Configuración</h4>
              <button class="text-white/80 hover:text-white transition-colors">
                <svg class="w-8 h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span class="block text-sm">Preferencias</span>
              </button>
            </div>
          </div>
        </div>
